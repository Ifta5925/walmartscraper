...
router.get('/reset/:token', (req, res) => {
    User.findOne({
        ...
        .then(user => {
            if (!user) {
                req.flash('error_msg', 'Password reset token invalid or expired')
                res.redirect('/forgot')
            }
...
router.post('/password/change', (req, res) => {
    if (req.body.password !== req.body.confirmpassword) {
        req.flash('error_msg', "Password doesn't match. Type again")
...
router.post('/forgot', (req, res, next) => {
    ...
    async.waterfall([
        ...
        (token, user) => {
            ...
            let mailOptions = {
                to: user.email,
                from: 'Hok Yang hokyang6@gmail.com',
    ...
    async.waterfall([
        ...
        (user) => {
            ...
            let mailOptions = {
                to: user.email,
                from: 'Hok Yang hokyang6@gmail.com',
                subject: 'Your password has been chamged',
                text: 'Hello,' + user.name + '/n/n' +
                    'This is the confirmation that' + user.email + ' has been changed'
            }
            smtpTransport.sendMail(mailOptions, err => {
